...
![ldmatrix 把 8x8 共享内存分块加载进 warp 寄存器片段;Ampere 上的反向通路使用普通存储,Hopper 上后来出现了专用的 stmatrix 指令](../img/ldstmatrix.svg)
...
![在朴素的行主序分块上,行写入在各 bank 间铺开,而列读取撞在同一个 bank 上;XOR swizzle 在不放弃合并的行写入的前提下,把列读取散布到各 bank](../img/swizzle_conflict.svg)
...
![Hopper 的共享内存矩阵描述符把操作数坐标映射到 swizzled 共享内存原子:描述符的步长选出原子,而 swizzle 选定原子内的字节位置](../img/smem_descriptor.svg)
...
![scale_vec 字节打包:1X 把一个缩放值在整个 4 字节单元中广播;2X 打包两个缩放值,各复制一份;4X 打包四个 K 块缩放值](../img/sf_scale_vec.svg)
