...
单 warpgroup 内核留下性能空间的原因很简单:每个线程走同一条路径——先加载、再计算、再回写。加载时 Tensor Core 空闲,计算时 TMA 引擎空闲。解决办法是*线程束特化(warp specialization)*。我们不再让一队线程轮流承担每项工作,而是把每项工作交给专职 warp,让这些 warp 同时运行,再由一条软件流水线把它们连接起来。这是 GEMM 路径上最大的架构变化,本章余下部分都建立在它之上。这里的基准测试取 M=N=K=4096。

> **本步改变了什么:作用域**
> - 作用域:一个 warpgroup 顺序执行加载 → MMA → 回写,变成由满/空屏障连接的三个并发角色(TMA 生产者、MMA 消费者、回写)。
> - 布局:不变,沿用第 6 步的 SMEM 流水级与 TMEM 累加器。
> - 派发:不变,TMA 加载,`tcgen05` MMA。

**主题。**

- 线程束特化(warp specialization):把不同的 warp/warpgroup 专门用于不同任务
...
在介绍角色与屏障之前,先单独看清线程束特化所移除的调度瓶颈会很有帮助。下图用第 4 步风格的串行时间线,作为第 4–6 步中特化前内核的紧凑参照,再把它放在第 7 步 warp 特化调度的上方,使引擎利用率差异一目了然。

![线程束特化时间线](../img/zh/warp_specialization_timeline.png)
...
图中有一个细节乍看像是错误:`mma2tma` 的箭头会跳过一个流水级。原因在于环形缓冲。在 `PIPE_DEPTH=2` 时有两个 SMEM 缓冲,即 0 号和 1 号流水级;TMA Load k=0 填充缓冲 0,TMA Load k=1 填充缓冲 1。当 MMA Compute k=0 读完缓冲 0 后,它会发出 `mma2tma` 信号表示该缓冲已空闲,但真正需要重新使用缓冲 0 的加载是 TMA Load k=2,而不是 k=1(k=1 用的是缓冲 1)。这就是为什么从 MMA Compute k=0 发出的 `mma2tma` 箭头会一直延伸到 TMA Load k=2。释放信号跳过一个流水级,纯粹是因为这个环有两个槽位。
...
四个屏障告诉各角色某个缓冲*何时*就绪;但当流水线循环时,还需要跟踪每个角色*正在使用*哪个缓冲。这种簿记正是 `PipelineState` 管理的内容。环形缓冲同时带着两份状态:当前在哪个槽位,以及等待的是该槽位屏障的哪个"相位"。在流水化循环里手工同时跟踪这两者,很容易产生差一错误,而这里一个差一就会让整个内核死锁。`PipelineState` 把两者绑定在一起,避免手写这部分状态管理:
...
线程束特化引入了一种容易踩中的同步隐患。一旦每个 warpgroup 运行不同的代码路径,熟悉的 `cta_sync()` 就会死锁:它使用硬件屏障 #0 并要求*每个* CTA 线程都到达,然而在 warpgroup 分支内只有其中一部分线程在场。我们真正需要的是一个作用域限于单个 warpgroup 的屏障。GPU 提供了 16 个具名屏障(ID 0–15),因此这些内核改用 `warpgroup_sync(10)`,它只同步一个 warpgroup 内的线程。当多个 warpgroup 各自需要独立同步时(如多消费者的第 9 步),它们通过 `warpgroup_sync(wg_id + 10)` 取用不同的 ID,从而不会碰到同一个硬件屏障。
...
**为何 A 和 B 跨集群拆分**:要看清 256×256 分块如何划分,回忆本教程把 GEMM 存储为 `D = A @ B.T`,其中存储的 B 形状为 `N x K`。集群中有两个 CTA,划分方式由此自然得到:
...
这里值得确认这确实带来收益,而不仅仅是重新安排工作。每个 CTA 仍只加载 128×K 的 A 和 128×K 的 B,所以集群整体暂存的操作数大约是单个 CTA 的 2×;但它产出 256×256 的分块,携带约 4× 于 128×128 分块的输出 FLOPs。因此,MMA 对每字节已暂存操作数执行的工作大约翻倍,因为每个 CTA 的 B 切片会通过协作式 MMA 与另一个 CTA 的 A 切片配合复用。换句话说,算术强度大约翻倍,而这正是一个仍偏访存的内核所需要的杠杆:端到端表中约 2.2× 的加速来自同样的字节支撑了更多运算。
...
两个 CTA 处理*同一个* 256×256 集群分块,而单一坐标 `cbx`(CTA 在集群内的位置,0 或 1)正是沿两条轴选出本 CTA 贡献的关键。`m_st` 选出本 CTA 拥有的输出行带,`n_st` 选出它提供给协作式 MMA 的存储-B 切片,而稍后的回写则发出 256 列输出跨度中的两段 128 列。还要注意,`num_m_tiles = M // 256` 和 `num_n_tiles = N // 256` 数的是集群分块,而不是单个 CTA 分块。
...
![GEMM 优化历程](../img/zh/gemm_perf.png)
